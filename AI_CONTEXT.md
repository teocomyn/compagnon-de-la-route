...
- Le hero de l'accueil utilise un slider plein cadre avec les quatre photographies fournies par le client dans `public/images/hero` : route nocturne, autocar en montagne, conducteur au poste et bus urbain en mouvement. Sur mobile, le slider précède le titre de marque « Compagnon de la route, plus qu'un métier, une mission ». Le défilement est contrôlable, se suspend au survol et au focus, accepte le swipe et les flèches du clavier, et devient manuel avec `prefers-reduced-motion`. Deux CTA explicites et les preuves réglementaires factuelles restent présents, sans promesse d'emploi, de financement ou de délai.
...
- `BorderBeamPanel` fournit une bordure orbitale aux couleurs orange et crème. Son usage est limité au Bento central de l'accueil et au formulaire de contact ; elle ralentit au repos, accélère au survol, se suspend hors écran et se fige avec `prefers-reduced-motion`.
