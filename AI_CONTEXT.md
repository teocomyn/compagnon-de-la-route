...
- Le hero de l'accueil parle du label entier et son CTA principal pointe vers `/formations`. Immédiatement après la ligne de réseau, la section `Pathways` distingue les deux métiers « à bord » et « à l'exploitation » avec leur statut public. L'ancienne section générique `WhyBecome` n'est plus rendue sur l'accueil afin de raccourcir la page et d'éviter de recentrer tout le récit sur le seul conducteur.
- La section `TrustBuilding` formalise la méthode BOAZ autour de quatre réalités : candidat, métier, entreprise lorsqu'elle est associée, et organisme. Elle ne garantit pas qu'un employeur est déjà engagé sur chaque candidature.
